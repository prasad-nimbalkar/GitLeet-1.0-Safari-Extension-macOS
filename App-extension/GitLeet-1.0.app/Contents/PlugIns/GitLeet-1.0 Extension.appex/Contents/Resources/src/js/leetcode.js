...
async function updateReadmeTopicTagsWithProblem(topicTags, problemName) {
  ...
  const { gitleet_token, gitleet_hook, stats } = await chrome.storage.local.get([
    'gitleet_token',
    'gitleet_hook',
    'stats',
  ]);
  ...
  for (const topic of topicTags) {
    readme = await appendProblemToReadme(topic.name, readme, gitleet_hook, problemName);
  }
...
function uploadGit(
  ...
  let useLanguageFolder = false;
    ...
    return chrome.storage.local
      ...
      .then(({ gitleet_token }) => {
        token = gitleet_token;
        if (gitleet_token == undefined) {
          throw new Error('gitleet token is undefined');
        }
        return chrome.storage.local.get('mode_type');
      })
      .then(({ mode_type }) => {
        if (mode_type !== 'commit') {
          throw new Error('gitleet mode is not commit');
        }
        return chrome.storage.local.get('gitleet_hook');
      })
      .then(({ gitleet_hook }) => {
        hook = gitleet_hook;
        if (!hook) {
          throw new Error('gitleet hook not defined');
        }
...
function LeetCodeV1() {
  this.progressSpinnerElementId = 'gitleet_progress_elem';
  this.progressSpinnerElementClass = 'gitleet_progress';
      ...
      for (let i = 0; i < scripts.length; i++) {
        ...
        if (text.includes('pageData')) {
          ...
          if (!commitMsg) {
            ...
            commitMsg = `Time: ${resultRuntime}, Memory: ${resultMemory} - GitLeet`;
          }
          if (code != null) {
            return uploadGit(
...
LeetCodeV1.prototype.startSpinner = function () {
  try {
    let elem = document.getElementById('gitleet_progress_anchor_element');
    if (!elem) {
      elem = document.createElement('span');
      elem.id = 'gitleet_progress_anchor_element';
...
/* Creates a ✔️ tick mark before "Run Code" button signaling GitLeet has done its job */
...
query submissionDetails($submissionId: ID!) {
  ...
  console.info('GitLeet:', { submissionDetailsData });
...
const loader = (leetCode, suffix) => {
  ...
  const intervalId = setInterval(async () => {
    try {
      ...
      const probStatsCommitMsg = `Time: ${probStats.time} (${probStats.timePercentile}%), Space: ${probStats.space} (${probStats.spacePercentile}%) - GitLeet`; // default commit
