...
$('#collapsible-commit-message-icon').click(() => {
  ...
  chrome.storage.local.get(['custom_commit_message'], data => {
    ...
    if (!commitMessage) {
      $('#custom-commit-msg').attr('placeholder', 'Time: {time}, Space: {space} - GitLeet');
    } else {
...
$('#msg-save-btn').click(() => {
  ...
  $('#custom-commit-msg').attr('placeholder', 'Time: {time}, Space: {space} - GitLeet'); // reset to default
        ...
        if (xhr.status === 200) {
          ...
          chrome.storage.local.get('mode_type', data2 => {
            if (data2 && data2.mode_type === 'commit') {
              ...
              chrome.storage.local.get(['stats', 'gitleet_hook'], data3 => {
                const { stats } = data3;
