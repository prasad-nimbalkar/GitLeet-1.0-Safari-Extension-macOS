...
const createRepo = (token, name) => {
  ...
  let data = {
    ...
    auto_init: true,
    description:
      'Collection of LeetCode questions to ace the coding interview! - Created using [GitLeet-1.0](https://github.com/prasad-nimbalkar/GitLeet-1.0)',
  };
      ...
      if (xhr.status === 200) {
        ...
        if (!bool) {
          ...
          chrome.storage.local
            .set({ gitleet_hook: res.full_name })
            .then(() => {
              console.log('Successfully set new repo hook');
...
$('#sync_counts').on('click', async () => {
  ...
  chrome.storage.local.get('gitleet_hook', data => {
    const hook = data.gitleet_hook;
    if (!hook) {
  ...
  const repo = await chrome.storage.local.get('gitleet_hook').then(({ gitleet_hook }) => {
    if (gitleet_hook == null) {
      $('#error').text('No repository linked - Please link a repository to sync counts!');
      $('#error').show();
      return;
    } else {
  ...
  const token = await chrome.storage.local.get('gitleet_token').then(({ gitleet_token }) => {
    if (gitleet_token == null) {
      $('#error').text('No token found - Please authorize GitLeet to access your GitHub account!');
      $('#error').show();
      return;
    } else {
      $('#error').hide();
    }
    return gitleet_token;
  });
